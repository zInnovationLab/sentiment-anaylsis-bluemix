...
module.exports.process_msg = function(ws, data){
	if(data.v === 1){
		if(data.type == 'create'){
			...
			if(data.name && data.term && data.size && data.user){
				chaincode.invoke.init_term([data.name, data.term, data.size, data.user], cb_invoked);				//create a new term
			}
		}
		else if(data.type == 'get'){
			console.log('get term msg');
		...
		if(e != null) console.log('error:', e);
			...
			try{
				...
				async.eachLimit(keys, concurrency, function(key, cb) {
					...
					chaincode.query.read([json[key]], function(e, term) {
						if(e != null) console.log('error:', e);
						else {
							sendMsg({msg: 'terms', e: e, term: term});
							cb(null);
						}
					});
				}, function() {
