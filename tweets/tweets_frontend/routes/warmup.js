...
if(process && process.env && process.env.VCAP_SERVICES) {
  ...
  var mongoUrl = 'mongodb://mongo_server:27017/tweets';
}
...
module.exports = function (app, socket) {
     ...
     */
    app.get('/tweets-sentiment', function (req, res) {
        res.render('tweets-sentiment', {});
    });
...
function alchemySearch(socket, db, term) {
    ...
    collect.find({ 'text': { '$regex': term, '$options': 'i' }}).toArray(function(err,result){
        if (err) {
            console.log ("ERROR: tweets.find: ", err);
        } else {
                ...
                (function(tweet) {
                    request({
                        ...
                        if(error) {
                            ...
                            tweet.sentiment = JSON.parse(body);
                            tweet.sentiment = tweet.sentiment['type'].toUpperCase();
                            //console.log(tweet);
